...
- **Jest API route tests** often mock `@/lib/transactions` or Prisma; **passing `npm test` alone does not prove** monthly-entry or other UI save flows work. After transaction API/lib changes, add tests that run real helper code (or verify with the same request body as the browser) before claiming the bug is fixed; see `.cursor/rules/verification-jest-vs-ui-api-paths.mdc`.
- If Prisma throws `Unknown argument '<newField>'` after schema updates, run `npx prisma generate` and restart the dev server; this is usually Prisma client/schema drift, not a DB data issue.
...
- Budget progress: `getBudgetIndicator` in `lib/budget-shared.ts` treats exactly 100% of limit as indicator `full`; `over` applies only when progress is greater than 1. Modal financial-account selection should use the drill-down slide picker in `transaction-form-dialog` (`AccountSlidePicker`) and reset picker navigation when the dialog closes.
...
- **Prisma migrations on MySQL/MariaDB (Linux):** `migration.sql` must use the same table identifier casing as tables created by earlier migrations (often PascalCase, e.g. `FinancialAccount`, `Transaction`). Case-only mismatches can apply on Windows but fail on case-sensitive servers, leaving expected columns missing.
- **Spending efficiency (daily pace):** dashboard route `/dashboard/spending-efficiency` (TH title ประสิทธิภาพการใช้เงิน); compares daily `EXPENSE` + `INTEREST` in approximate THB to a user-set daily target; data via `GET /api/spending-efficiency` (`from`, `to`, `timezone`); optional persisted target in `localStorage` key `judtang_spending_efficiency_daily_target`.
- **Recurring “paid this month”:** `isPaid` for a template is based only on posted rows that have that template’s `recurringTransactionId` in the due calendar month; a manual transaction without that link does not count as paying the recurring round (confirming recurring without removing the manual row can double-count).
- **Debit card + linked bank (`linkedAccountId`):** balance-after snapshots for postings on the card leg must follow a combined chronological ledger across the **card and linked bank account**; rebuilding snapshots for the bank alone can leave `accountBalanceAfter` / `transferAccountBalanceAfter` unset on card rows and the transactions list shows “—” until card+linked rebuild runs.
